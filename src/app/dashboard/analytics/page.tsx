...
async function handleLogout() {
  "use server";
  const { createServerClient: sc } = await import("@supabase/ssr");
  ...
  const sb = sc(
    ...
    { cookies: { getAll: () => cs.getAll(), setAll: (toSet) => toSet.forEach(({ name, value, options }) => cs.set(name, value, options)) } }
  );
...
export default async function AnalyticsPage() {
  ...
  const { data: profile } = await supabase
    .from("users")
    .select("plan, trial_expires_at, reward_plan, reward_expires_at")
    .eq("id", user.id)
    .single();

  const effectivePlan = getEffectivePlan(profile ?? {});
  const navIsPro   = effectivePlan === "pro" || effectivePlan === "ultima";
  const navIsUltima = effectivePlan === "ultima";
