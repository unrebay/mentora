      ...
      <div className="max-w-5xl mx-auto px-6 py-10">
        ...
        {!isPro && !profile?.streak_reward_claimed && maxStreak < 7 && (
          <div className="mb-6 flex items-center gap-4 bg-orange-50 border border-orange-200 rounded-2xl px-5 py-4">
            <span className="text-2xl">🔥</span>
            <div className="flex-1">
              <p className="font-semibold text-orange-800 text-sm">{maxStreak === 0 ? "Начни стрик — и получи Pro бесплатно" : `${maxStreak} из 7 дней → 3 дня Pro`}</p>
              <p className="text-xs text-orange-600 mt-0.5">Учись {7 - maxStreak}{" "}{7 - maxStreak === 1 ? "день" : 7 - maxStreak < 5 ? "дня" : "дней"} подряд и получи 3 дня Pro без карты.</p>
              <div className="mt-2 h-1.5 rounded-full bg-orange-100 overflow-hidden w-full max-w-xs"><div className="h-full rounded-full bg-orange-400 transition-all" style={{ width: `${Math.round((maxStreak / 7) * 100)}%` }} /></div>
            </div>
        ...
        {!isPro && profile?.streak_reward_claimed && !isTrialActive && (
          <div className="mb-6 flex items-center gap-3 bg-[var(--bg-secondary)] border border-[var(--border)] rounded-2xl px-5 py-4">
            ...
            <div>
              ...
              <p className="text-xs text-[var(--text-secondary)] mt-0.5">Понравилось? Оформи полную подписку и учись без лимитов.{" "}<a href="/pricing" className="text-brand-600 font-medium hover:underline">Посмотреть тарифы →</a></p>
            </div>
          </div>
        )}
        <div className="mb-10">
          ...
          {!isPro && (
            <div className="flex flex-wrap gap-3">
              <div className="flex items-center gap-2 bg-[var(--bg-card)] border border-[var(--border)] rounded-xl px-4 py-2.5 text-sm">
                ...
                <span className="text-[var(--text-secondary)]">Сообщений сегодня:{" "}<span className={`font-semibold ${messagesRemaining === 0 ? "text-red-500" : messagesRemaining !== null && messagesRemaining <= 5 ? "text-orange-500" : "text-[var(--text)]"}`}>{messagesRemaining} / {DAILY_LIMIT}</span></span>
              </div>
              ...
              {maxStreak > 0 && <div className="flex items-center gap-2 bg-[var(--bg-card)] border border-[var(--border)] rounded-xl px-4 py-2.5 text-sm"><span>🔥</span><span className="text-[var(--text-secondary)]">Стрик:{" "}<span className="font-semibold text-orange-500">{maxStreak} {pluralDays(maxStreak)}</span></span></div>}
              <Link href="/pricing" className="flex items-center gap-2 bg-brand-50 border border-brand-200 rounded-xl px-4 py-2.5 text-sm text-brand-700 font-medium hover:bg-brand-100 transition-colors">✨ Убрать лимит</Link>
            </div>
          )}
          {isPro && (
            <div className="flex flex-wrap gap-3">
