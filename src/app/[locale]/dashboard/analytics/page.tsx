...
export default async function AnalyticsPage() {
  ...
  // best = max(account best, any historical per-subject best) so existing users
  // never lose an earned streak achievement when switching to the visit streak.
  const { data: streakRow } = await supabase
    .from("users")
    .select("visit_streak, visit_streak_best")
    .eq("id", user.id)
    .single();
  const currentStreak  = streakRow?.visit_streak ?? 0;
  const bestStreak     = Math.max(
    streakRow?.visit_streak_best ?? 0,
    progress.reduce((m, r) => Math.max(m, r.best_streak ?? r.streak_days ?? 0), 0),
  );
