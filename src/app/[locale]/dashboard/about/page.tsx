...
export default function AboutPage() {
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const uniqueData = t.raw("unique") as { title: string; desc: string }[];
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  ...
  return (
    ...
    >
      ...
      <div className="relative" style={{ zIndex: 1 }}>
        ...
        {/* ── UNIQUENESS — что нас отличает (positive self-positioning) ─── */}
        <section className="max-w-4xl mx-auto px-5 sm:px-8 pb-14">
          <Tag color="#4561E8">{t("uniqueTag")}</Tag>
          <h2 style={{ fontSize: 26, fontWeight: 900, color: "var(--text)", margin: "16px 0 6px", letterSpacing: "-0.5px" }}>
            {t("uniqueTitle")}
          </h2>
          ...
          <div className="space-y-3">
            {uniqueData.map((u, idx) => {
              const accents = ["#4561E8", "#7C3AED", "#10B981", "#F59E0B", "#FF7A00"];
              const accent = accents[idx % accents.length];
              return (
                <div key={u.title}
                  className="relative rounded-2xl overflow-hidden flex items-start gap-4 p-5"
                  style={{
                ...
                >
                  {/* number chip */}
                  <div
                    className="shrink-0 flex items-center justify-center rounded-xl font-black text-sm"
                    style={{
                      width: 40, height: 40,
                      background: `linear-gradient(135deg, ${accent}28, ${accent}0F)`,
                  ...
                  <div className="flex-1 min-w-0">
                    <div style={{ fontWeight: 700, color: "var(--text)", fontSize: 14, marginBottom: 5 }}>
                      {u.title}
                    </div>
