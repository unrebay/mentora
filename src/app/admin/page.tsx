  ...
  return (
    <div className="min-h-screen bg-[#06060f] text-white">
      ...
      <main className="max-w-7xl mx-auto px-6 py-8 space-y-8">
        ...
        <section>
          <h2 className="text-xs font-semibold text-white/40 tracking-[0.15em] uppercase mb-3">Роадмап</h2>
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            {([
              {
                phase: "Фаза 0 · Сейчас",
                label: "Тест с друзьями",
                color: "border-blue-500/30 bg-blue-500/5",
                accent: "text-blue-400",
                dot: "bg-blue-400",
                items: [
                  { done: true,  text: "3D карусель предметов на лендинге" },
                  { done: true,  text: "Скролл-шапка + демо-лимиты (3/5 msg)" },
                  { done: true,  text: "Миссия: «каждому», школьника, без лейбла" },
                  { done: true,  text: "Mentora scope — вся наука, не только школа" },
                  { done: true,  text: "Безопасность: 4 уязвимости закрыты" },
                  { done: false, text: "QA-агент: систематический прогон страниц" },
                  { done: false, text: "8h rolling window для Free тарифа" },
                ],
              },
              {
                phase: "Фаза 1 · До 31 мая",
                label: "Первая конверсия",
                color: "border-yellow-500/30 bg-yellow-500/5",
                accent: "text-yellow-400",
                dot: "bg-yellow-400",
                items: [
                  { done: false, text: "Редизайн /pricing — premium карточки" },
                  { done: false, text: "Промо «-30% только до 31 мая»" },
                  { done: false, text: "«Задача дня» — возврат без стрика" },
                  { done: false, text: "Редизайн ChatInterface" },
                  { done: false, text: "Редизайн /onboarding" },
                  { done: false, text: "Обновить GitHub PAT (истекает 22 мая)" },
                  { done: false, text: "hi@mentora.su → hello@ в terms/page" },
                ],
              },
              {
                phase: "Фаза 2 · 1 июня",
                label: "Официальный запуск",
                color: "border-purple-500/30 bg-purple-500/5",
                accent: "text-purple-400",
                dot: "bg-purple-400",
                items: [
                  { done: false, text: "Подарок 1 мес Pro всем зарегистрированным" },
                  { done: false, text: "ProExpiryBanner — давление на конверсию" },
                  { done: false, text: "Публичный профиль со стриком + шаринг" },
                  { done: false, text: "Реферальная пирамида — визуальное усиление" },
                  { done: false, text: "Первые 100 платящих → ЕГЭ-режим" },
                ],
              },
            ] as { phase: string; label: string; color: string; accent: string; dot: string; items: { done: boolean; text: string }[] }[]).map(({ phase, label, color, accent, dot, items }) => (
              <div key={phase} className={`rounded-2xl border p-4 ${color}`}>
                <div className="flex items-center gap-2 mb-3">
                  <div className={`w-2 h-2 rounded-full flex-shrink-0 ${dot}`} />
                  <div>
                    <div className={`text-xs font-semibold ${accent}`}>{phase}</div>
                ...
                <div className="space-y-2">
                  {items.map((item, i) => (
                    <div key={i} className="flex items-start gap-2">
                      <div className={`w-4 h-4 rounded flex-shrink-0 mt-0.5 flex items-center justify-center text-[10px] ${item.done ? "bg-green-500/20 text-green-400" : "bg-white/5 text-white/20"}`}>
                        {item.done ? "✓" : "·"}
                      </div>
                      <span className={`text-xs leading-tight ${item.done ? "text-white/30 line-through" : "text-white/60"}`}>{item.text}</span>
                    </div>
                  ))}
                </div>
                <div className="mt-3 pt-3 border-t border-white/5 text-xs text-white/25">
                  {items.filter(i => i.done).length}/{items.length} выполнено
                </div>
              </div>
            ))}
          </div>
