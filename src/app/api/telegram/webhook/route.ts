...
Правила поведения:
- Отвечай ТОЛЬКО по-русски или по-английски. НИКОГДА не используй китайские, японские, корейские и любые другие иероглифы — даже случайно. Если в тексте попадается иноязычный символ — удали его и замени русским словом. Дружелюбно, на «ты», по делу. Без реверансов и канцелярита.
...
async function handleUpdate(update: Record<string, unknown>) {
  ...
  // ── Greeting intercept — hardcoded, no AI cost ─────────────────────────
  const greetingPatterns = /^(привет|hello|hi|хай|хей|ку|прив|добрый день|добрый вечер|доброе утро|здравствуй|здравствуйте|👋|🙋|start|старт)[\.!\s]*$/i;
  if (greetingPatterns.test(text.trim())) {
    await sendMessage(chatId,
      `Привет! 👋 Я AI-ассистент <b>Mentora</b> — помогаю с платформой и учёбой.

` +
      `Что умею:
` +
      `— ответить на вопрос про сайт, тарифы, аккаунт
` +
      `— помочь с учёбой: объяснить тему, разобрать задачу, проверить решение
` +
      `— связать с командой если нужна живая помощь

` +
      `Просто напиши свой вопрос 👇
` +
      `<i>/help — частые вопросы · /reset — начать заново</i>`
    );
    return;
  }
