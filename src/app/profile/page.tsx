...
const BADGES: BadgeDef[] = [
  ...
  { id: "xp100",          icon: "⚡", name: "Первые шаги",       desc: "Набрал 100 XP",                     tier: "bronze",  check: s => s.totalXP >= 100       },
  { id: "xp500",          icon: "⚡", name: "Знаток",            desc: "Набрал 500 XP",                     tier: "silver",  check: s => s.totalXP >= 500       },
  { id: "xp1000",         icon: "💎", name: "Мастер",            desc: "Набрал 1000 XP",                    tier: "gold",    check: s => s.totalXP >= 1000      },
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <main className="max-w-3xl mx-auto px-6 py-10 space-y-8">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
          {[
            { label: "Всего XP",     value: totalXP,        icon: "⚡" },
            { label: "Макс. стрик",  value: `${maxStreak}д`, icon: "🔥" },
            { label: "Сообщений",    value: totalMessages,   icon: "💬" },
            { label: "Достижений",   value: earned.length,   icon: "🏅" },
          ].map(s => (
        ...
        <div className="bg-white rounded-2xl p-6 border border-gray-100 shadow-sm">
          <div className="flex items-center justify-between mb-3">
            ...
            {lvl.next && <span className="text-sm text-gray-400">{lvl.next.name} через {(lvl.next.minXP - totalXP)} XP</span>}
          </div>
          <div className="h-3 bg-gray-100 rounded-full overflow-hidden">
            <div className="h-full rounded-full transition-all duration-500"
                 style={{ width: `${lvl.progress}%`, background: lvl.color }} />
          </div>
          <p className="text-xs text-gray-400 mt-2">{totalXP} / {lvl.next?.minXP ?? totalXP} XP</p>
        </div>
          ...
          <div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
              {earned.map(b => {
                ...
                return (
                  <div key={b.id} className="bg-white rounded-2xl p-5 border-2 shadow-sm flex flex-col items-center text-center gap-2"
                    ...
                    <span className="text-[10px] font-semibold px-2 py-0.5 rounded-full mt-1"
                          style={{ background: ts.border + "22", color: ts.border }}>{ts.label}</span>
                  </div>
                );
              })}
            </div>
