    ...
    <div className="rounded-2xl border overflow-hidden"
      ...
      <div className="px-6 pt-5 pb-4 flex items-start justify-between gap-4">
        <div>
          <div className="flex items-center gap-2.5 mb-1">
            ...
            <h2 className="font-bold text-base" style={{ color: "var(--text)" }}>Поделиться прогрессом</h2>
          </div>
          <p className="text-sm" style={{ color: "var(--text-muted)", lineHeight: 1.5 }}>
            Создай ссылку для учителя или родителя — они увидят твою статистику в реальном времени
          </p>
        </div>
        <button
          onClick={() => setOpen(o => !o)}
      ...
      {open && (
        <div className="mx-6 mb-4 p-4 rounded-xl" style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(69,97,232,0.15)" }}>
          <p className="text-xs mb-2" style={{ color: "var(--text-muted)" }}>Имя получателя (необязательно)</p>
          <div className="flex gap-2">
            <input
              value={label}
              onChange={e => setLabel(e.target.value)}
              placeholder="Например: Мама, Учитель истории..."
            ...
            return (
              <div key={inv.id}
                ...
                <button
                  ...
                  {copied === token ? (
                    <><svg viewBox="0 0 24 24" width="12" height="12" fill="none" stroke="currentColor" strokeWidth="2.5"><polyline points="20 6 9 17 4 12"/></svg>Скопировано</>
                  ) : (
                    <><svg viewBox="0 0 24 24" width="12" height="12" fill="none" stroke="currentColor" strokeWidth="2"><rect x="9" y="9" width="13" height="13" rx="2"/><path d="M5 15H4a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h9a2 2 0 0 1 2 2v1"/></svg>Скопировать</>
                  )}
                </button>
                <button
